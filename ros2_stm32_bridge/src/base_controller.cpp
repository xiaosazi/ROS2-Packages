...
void MiniDriver::SendTimerCallback() {
  ...
  //机器人x轴的目标线速度
  transition = 0;
  transition = current_twist_.linear.x*1000; //将浮点数放大一千倍，简化传输
  data[5] = transition & 0xff;     //取数据的低8位
  data[4] = transition>>8;  //取数据的高8位

  //The target velocity of the Y-axis of the robot
  //机器人y轴的目标线速度
  transition = 0;
  transition = current_twist_.linear.y*1000;
  data[7] = transition & 0xff;
  data[6] = transition>>8;

  //The target angular velocity of the robot's Z axis
  //机器人z轴的目标角速度
  transition = 0;
  transition = current_twist_.angular.z*1000;
  data[9] = transition & 0xff;
  data[8] = transition>>8;
